...
def generate_integrity_report(
    ...
    # per-group chart (electrode//group_size — for fs437 these are the 4 organoids on one chip)
    wells = sorted(rep["per_group"].keys())
    well_labels = [f"Group {w}" for w in wells]
    ...
    rows_summary = f"""
      ...
      <tr><td>Electrodes / groups</td><td>{data.n_electrodes} / {rep['n_groups']}</td></tr>
    ...
    if contaminated:
        ...
        <ul>
          <li><b>Connectivity / functional networks are inflated.</b> Synchronous transients create spurious cross-electrode and cross-group "connections" that are not biology.</li>
          ...
          <li><b>Low-activity groups are most affected</b> — where real spikes are few, the artifact can dominate the recording.</li>
        </ul>"""

    methods = f"""
      <h2>Methods &amp; reproducibility</h2>
      <p class="methods">The signature of common-mode contamination is coincidence at the <i>exact same
      sample</i> across different electrode groups, skewed to high amplitude and often recruiting many
      groups at once — the hallmark of a shared reference / ground / medium rather than independent neural
      firing (whether the groups are separate wells or co-housed organoids on one chip). A spike is flagged
      when it coincides within ±{window_ms:.1f} ms with a spike on a different electrode group, or when an
      abnormal fraction of all electrodes fire in the same window. Significance is established against a
      jittered null (each group randomly time-shifted), reported as enrichment over chance. The amplitude
      dissociation (artifact spikes high-amplitude, biological spikes low-amplitude) is an independent
      corroboration, not an input to the flag. This is standard data-quality QC: common-average referencing
      or coincidence filtering before connectivity/complexity is routine in multi-electrode work.</p>
...
<body>
  ...
  <div class="sub">Common-mode artifact audit · multi-electrode MEA recording</div>
  ...
  <h2>Artifact by electrode group</h2>
  {_bar_chart(well_labels, well_vals, danger_above=40)}
  <div class="chartcap">Fraction of each group's spikes that are cross-group-synchronous (artifact). Red ≥ 40%.</div>
